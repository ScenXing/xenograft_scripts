...
vaf_raw <- read.csv("VAFs-control-G89HFMTissue-G89filteredPDX-passed.somatic.snvs.csv", sep=",", header = T, stringsAsFactors = FALSE, skip = 2)
...
png("VAF_patient_vs_PDXfiltered_trans.png", width=15, height=15, units="in", res=400)
...
ggplot(vaf, aes(x = VAF2_T1, y = VAF1_T1, color = Chromosome)) + geom_point(shape=19) +
    geom_point(alpha = 0.3) +
    scale_colour_manual(values=cbPalette) + 
    xlab("G89 Patient Tumour") + ylab("G89 PDX_filtered") + ggtitle("G89 vs G89PDX_filteredmouse")
