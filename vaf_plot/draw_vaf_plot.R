...
vaf_plot <- ggplot(vaf, aes(x = VAF2_T1, y = VAF1_T1, color = Chromosome, size = 10)) + geom_point(shape=19) +
    scale_colour_manual(values=cbPalette) + 
    geom_point(alpha = 0.3) +
    xlab("G89HFMTissue") + ylab("G89PDX") + ggtitle("G89 vs G89PDX")

ggsave("VAF5.png", vaf_plot, width=15, height=15, units="in")
#ggsave("VAF.png", vaf_plot, width=15, height=15, units="in", res=400)
#dev.off() 



## Test plot without ggplot. Shows that points form a strange line near the 50% VAF
#png(filename="NONGG_VAF1.png")
#non_gg <- plot(vaf$VAF2_T1, vaf$VAF1_T1, pch=20, cex=0.2)
#dev.off()
