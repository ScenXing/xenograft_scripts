...
SEQZ_DIR="/share/ScratchGeneral/julyin/G89/sequenza_normal_vs_mousefilt_pypy_more_mem/seqz_large_files"

BINNED_SEQZ_DIR="/share/ScratchGeneral/julyin/G89/sequenza_normal_vs_mousefilt_pypy_more_mem/bigger_binned_seqz"
...
$PYPY ${SEQ_UTILS}/sequenza-utils.py bam2seqz -n ${NORMAL_DIR}/$NORM.dedup.realn.bam -t ${TUMOUR_DIR}/$TUM.bam -F $HUMAN_REF -gc $GC_FILE -C chr$CHR | \
...
# tail -n +2: output the last K lines, instead of the last 10
    ...
    $PYPY ${SEQ_UTILS}/sequenza-utils.py seqz-binning -w 200 -s ${SEQZ_DIR}/$NORM.vs.$TUM.$CHR.large.seqz.gz | tail -n +2 | gzip > ${BINNED_SEQZ_DIR}/$NORM.vs.$TUM.$CHR.seqz.gz
