...
SNAKE_FILE="get_intersection_dbsnp_common.snakemake"
...
snakemake -s $SNAKE_FILE --cluster-config cluster.json --drmaa " -q all.q -cwd -V -b y -j y -pe smp {cluster.cores} -l {cluster.hvmem} -l {cluster.memrequested} -N {cluster.jobname}" -j 200
